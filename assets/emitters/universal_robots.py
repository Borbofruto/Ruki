...
def parse_script_moves(linhas):
    ...
    MoveL: retorna pose cartesiana [x,y,z,rx,ry,rz] em metros/rad
    """
    comandos = []
    ...
    for linha in linhas:
        ...
        if m:
            try:
                ...
                comandos.append({'cmd': 'move', 'tipo': 'MoveJ', 'joints': vals})
            except: pass
            continue
        ...
        if m:
            try:
                vals = [float(v.strip()) for v in m.group(1).split(',')][:6]
                comandos.append({'cmd': 'move', 'tipo': 'MoveL', 'pose': vals})
            except: pass
            continue
...
def script_para_urp(arquivo, robot_data, pasta):
    """Converte .script → .urp (MoveL usa pose cartesiana)"""
    ...
    for cmd in comandos:
        if cmd['cmd'] == 'move':
            ...
            if 'joints' in cmd:
                ...
                p = cmd['pose']
                ET.SubElement(pos, 'CartesianPose', pose=', '.join(f'{x:.6f}' for x in p))
...
def script_para_robodk(arquivo, robot_data, pasta):
    """Converte .script → .py (MoveL usa pose cartesiana)"""
    linhas = ler_arquivo(arquivo)
...
"""Programa RoboDK gerado por Ruki-C v1.6"""
...
def ur_pose_to_robodk(x, y, z, rx, ry, rz):
    """Converte pose UR (metros + rotation vector rad) para Pose RoboDK (mm + matriz)"""
    ...
    for cmd in comandos:
        if cmd['cmd'] == 'move':
            if cmd['tipo'] == 'MoveJ' and 'joints' in cmd:
                ...
                mc += 1
            elif cmd['tipo'] == 'MoveL' and 'pose' in cmd:
                p = cmd['pose']
